...
static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    if (!node) return true;
    // ensure it checks grandparents too
    if (node->left) {
        Node *left_max = max_element_impl(node->left);
        if (!less(left_max->datum, node->datum)) return false;
    }
    if (node->right) {
        Node *right_min = min_element_impl(node->right);
        if (!less(node->datum, right_min->datum)) return false;
    }
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    ...
    if(less(val, node->datum)){
      ...
      return node;
        ...
        // node is <= val, answer must be to the right
